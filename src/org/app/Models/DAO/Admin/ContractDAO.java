...
public class ContractDAO {
    ...
    public void delete(UUID id) throws SQLException {
        String sql = "UPDATE contracts SET deleted_at = ? WHERE id = ?";
    ...
    public Contract findById(UUID id) throws SQLException {
        try (Connection connection = databaseC.getInstance().getConnection()) {
            String sql = "SELECT * FROM contracts WHERE id = ? AND deleted_at IS NULL";
            PreparedStatement statement = connection.prepareStatement(sql);
    ...
    public List<Contract> getContracts() throws SQLException {
        ...
        try (Connection connection = databaseC.getInstance().getConnection()) {
            String sql = "SELECT * FROM contracts WHERE deleted_at IS NULL";
    ...
    public void save(Contract contract) throws SQLException {
        String sqlContract = "INSERT INTO contracts (id, initDate, endDate, specialTariff, accordConditions, renewed, currentStatus, partner_id) VALUES (?, ?, ?, ?, ?, ?, CAST(? AS currentStatus), ?)";
        try (Connection connection = databaseC.getInstance().getConnection()) {
            // Save the contract
    ...
    public void update(Contract contract) throws SQLException {
        String sql = "UPDATE contracts SET initDate = ?, endDate = ?, specialTariff = ?, accordConditions = ?, renewed = ?, currentStatus = ?, partner_id = ? WHERE id = ?";
        try (Connection connection = databaseC.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    private List<PromotionalOffer> getPromotionalOffersForContract(UUID contractId, Connection connection) throws SQLException {
        ...
        String sql = "SELECT po.* FROM promotionaloffers po " +
                "JOIN contract_promotionaloffers cpo ON po.id = cpo.promotional_offer_id " +
    ...
    private void savePromotionalOffers(Contract contract, Connection connection) throws SQLException {
        String sql = "INSERT INTO contract_promotionaloffers (contract_id, promotional_offer_id) VALUES (?, ?)";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    private void deletePromotionalOffers(UUID contractId, Connection connection) throws SQLException {
        String sql = "DELETE FROM contract_promotionaloffers WHERE contract_id = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setObject(1, contractId);
            statement.executeUpdate();
        }
