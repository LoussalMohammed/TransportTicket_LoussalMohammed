...
public class PromotionalOfferDAO {
    ...
    public void delete(UUID id) throws SQLException {
        String sql = "UPDATE promotionalOffers SET deleted_at = ? WHERE id = ?";
    ...
    public static PromotionalOffer findById(UUID id) throws SQLException {
        try (Connection connection = databaseC.getInstance().getConnection()) {
            String sql = "SELECT * FROM promotionalOffers WHERE id = ? AND deleted_at IS NULL";
            PreparedStatement statement = connection.prepareStatement(sql);
    ...
    public List<PromotionalOffer> getPromotionalOffers() throws SQLException {
        ...
        try (Connection connection = databaseC.getInstance().getConnection()) {
            String sql = "SELECT * FROM promotionalOffers WHERE deleted_at IS NULL";
    ...
    public void save(PromotionalOffer offer) throws SQLException {
        String sql = "INSERT INTO promotionalOffers (id, name, description, initDate, endDate, reductionType, reductionValue, conditions, offerStatus) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = databaseC.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public void update(PromotionalOffer offer) throws SQLException {
        String sql = "UPDATE promotionalOffers SET name = ?, description = ?, initDate = ?, endDate = ?, reductionType = ?, reductionValue = ?, conditions = ?, offerStatus = ? WHERE id = ?";
        try (Connection connection = databaseC.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
